    ...
    def execute_command(self, command: Any, context: Any) -> Any:
        ...
        if isinstance(command, NavigationCommand):
            ...
            if command.action == 'click':
                ...
                if isinstance(target, LocatorResult) and target.diagnostics.get("element") is not None and hasattr(self.accessibility_provider, "invoke_pattern"):
                    self.accessibility_provider.invoke_pattern(target.diagnostics["element"], "invoke")
                    return {"status": "SUCCESS", "provider": self.accessibility_provider.__class__.__name__}
